...
import {
  ...
  faFileCode, faArrowLeft, faArrowUp, faArrowDown, faTimes
...
const HighlightText = ({ text, query, globalStartIndex, currentActiveMatch, isLineActive }: { text: string, query: string, globalStartIndex: number, currentActiveMatch: number, isLineActive: boolean }) => {
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      ...
      if (msg.type === 'updateProjects') {
        ...
        (msg.data || []).forEach((p: any) => { if (p.branch) initialBranches[p.fsPath] = p.branch; });
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {contextMenu.type === 'top' && (
              <>
                ...
                <li onClick={() => executeMenuAction('searchInFolder')}><FontAwesomeIcon icon={faMagnifyingGlass} className="menu-icon" /> 查找文件内容...</li>
                <div className="menu-separator"></div>
      ...
      {isSearchMode && searchTargetProject ? (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
          <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--vscode-panel-border)' }}>
            ...
            <div className="search-box" style={{ padding: '2px 4px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', paddingRight: '4px' }}>
                ...
                <button className="action-btn-icon" style={{ padding: '2px 4px' }} onClick={handlePrevSearchMatch} disabled={totalMatches === 0}><FontAwesomeIcon icon={faArrowUp} /></button>
                <button className="action-btn-icon" style={{ padding: '2px 4px' }} onClick={handleNextSearchMatch} disabled={totalMatches === 0}><FontAwesomeIcon icon={faArrowDown} /></button>
              </div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px' }}>
            {isSearchingFolder ? (
              <div style={{ textAlign: 'center', padding: '20px', opacity: 0.7 }}><FontAwesomeIcon icon={faSpinner} spin /> 正在高速检索中...</div>
            ) : folderSearchError ? (
                ...
                {folderSearchResults.map((res, i) => (
                  <li key={i} style={{ marginBottom: '8px' }}> 
                    <div style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--vscode-textLink-foreground)', marginBottom: '2px', wordBreak: 'break-all' }}>
                      <FontAwesomeIcon icon={faFileCode} style={{ marginRight: '6px' }} />{res.file}
                    </div>
                      ...
                      {res.matches.map((m: any, j: number) => {
                        ...
                        return (
                          <li key={j}
                            ...
                            onMouseEnter={(e) => { if (!isLineActive) e.currentTarget.style.backgroundColor = 'var(--vscode-list-hoverBackground)' }}
                            onMouseLeave={(e) => { if (!isLineActive) e.currentTarget.style.backgroundColor = 'transparent' }}
                          >
                            <span style={{ color: isLineActive ? 'inherit' : 'var(--vscode-descriptionForeground)', opacity: 0.8, marginRight: '8px', display: 'inline-block', minWidth: '24px', textAlign: 'right' }}>{m.line}</span>
