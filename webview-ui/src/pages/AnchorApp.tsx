...
function getIconSvg(iconDef: any, className: string = '') {
...
export default function AnchorApp() {
  ...
  const hoverTimeout = useRef<any>(null); // Tooltip 悬浮定时器
  ...
  useEffect(() => {
    ...
    const diagonal = d3.linkHorizontal<any, d3.HierarchyPointNode<TreeNodeData>>()
      .x((d) => d.y)
      .y((d) => d.x);
    ...
    if (tooltipEl) {
      ...
      tooltipEl.addEventListener('mouseenter', () => clearTimeout(hoverTimeout.current));
    ...
    function update(source: TreeNode) {
      ...
      nodeEnter.append("path")
        ...
        .attr("d", faLink.icon[4] as string)
      ...
      // ----- 连线 -----
      const link = g.selectAll<SVGPathElement, d3.HierarchyLink<TreeNodeData>>("path.link")
        .data(links, (d) => (d.target as TreeNode).id as string);
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message.command === 'refresh' && message.data) {
        ...
        // 动态获取高度，如果没有挂载，给个默认值 window.innerHeight
