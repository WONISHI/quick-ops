...
function buildGraphEngine(commits: GraphCommit[]) {
  ...
  commits.forEach((c, i) => {
    ...
    const parents = c.type === 'stash' ? (c.parents || []).slice(0, 1) : c.parents || [];
...
const GitGraph: React.FC<GitGraphProps> = ({
  ...
  const [commitChangeStatsMap, setCommitChangeStatsMap] = useState<Record<string, { filesChanged: number; insertions: number; deletions: number }>>({});
  ...
  const matchedIndices = useMemo(() => {
    ...
    return graphCommits
      .map((c, i) => {
        const matched = getCommitDisplayMessage(c).toLowerCase().includes(lowerQuery) || c.author.toLowerCase().includes(lowerQuery) || c.hash.toLowerCase().includes(lowerQuery);
  ...
  useEffect(() => {
    ...
    graphData.vertices.slice(0, displayCount).forEach((v, idx) => {
      ...
      const dotColor = isUncommittedVertex ? '#808080' : vBranch ? COLORS[vBranch.colour % COLORS.length] : '#808080';
  ...
  const handleMouseEnter = (e: React.MouseEvent, commit: GraphCommit) => {
    ...
    hoverTimeoutRef.current = setTimeout(() => {
      ...
      if (commit.type !== 'stash' && typeof commit.filesChanged !== 'number' && !commitChangeStatsMap[commit.hash]) {
        vscode.postMessage({
  ...
  const highlightText = (text: string, query: string, isActiveMatch: boolean) => {
    ...
    return parts.map((part, i) => {
      return part.toLowerCase() === query.toLowerCase() ? (
        <span key={i} className={`${styles['graph-search-highlight']} ${isActiveMatch ? styles['graph-search-highlight-active'] : ''}`}>
          {part}
        </span>
      ) : (
        part
                    ...
                    >
                      ...
                      <div className={styles['commit-content']}>
                        ...
                        <div className={styles['commit-row-actions']}>
                          {localRef && (
                            <div className={styles['local-ref-badge']} title={`本地分支: ${localRef}`}>
                          ...
                          {isRemotePush && (
                            <div className={styles['remote-sync-badge']} title="已同步至远程仓库">
