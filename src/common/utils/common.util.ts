...
import type { DisposableLike, MaybePromise, PathInfo, QuickPickOption, VscodeCommandHandler, WebviewResourceOptions } from '@common/types/common.type';
...
export async function safeCall<T>(fn: () => MaybePromise<T>, fallback: T, onError?: (error: unknown) => void): Promise<T> {
  try {
    return await fn();
  } catch (error) {
    onError?.(error);
...
export async function safeDisposeAll(disposables: Array<DisposableLike | null | undefined>): Promise<void> {
...
export function registerCommands(
  ...
  return commands.map((item) => registerCommand(context, item.command, item.handler, item.thisArg));
}
...
export function getRelativePath(targetPath: string, rootPath = getWorkspaceRoot()): string {
...
export async function readJsonFile<T = unknown>(uri: vscode.Uri, fallback: T): Promise<T> {
  try {
...
export async function writeJsonFile<T = unknown>(uri: vscode.Uri, data: T, space = 2): Promise<void> {
...
export function createQuickPickOption<T>(label: string, value: T, options?: Omit<QuickPickOption<T>, 'label' | 'value'>): QuickPickOption<T> {
  return {
    label,
    value,
    ...options,
...
export async function showQuickPickValue<T>(items: QuickPickOption<T>[], options?: vscode.QuickPickOptions): Promise<T | undefined> {
...
export function getWebviewUri(options: WebviewResourceOptions): vscode.Uri {
  return options.webview.asWebviewUri(vscode.Uri.joinPath(options.extensionUri, ...options.paths));
}
...
export function debounce<TArgs extends any[]>(fn: (...args: TArgs) => void, wait = 300): (...args: TArgs) => void {
...
export function throttle<TArgs extends any[]>(fn: (...args: TArgs) => void, wait = 300): (...args: TArgs) => void {
  let lastTime = 0;
  let timer: NodeJS.Timeout | undefined;
...
export function uniqueBy<T>(list: T[], getKey: (item: T) => string | number): T[] {
