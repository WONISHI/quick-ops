...
export class LocalProxyServerService {
  ...
  private isJavascriptResponse(headers: http.IncomingHttpHeaders): boolean {
    ...
    return contentType.includes('javascript') || contentType.includes('ecmascript') || contentType.includes('application/x-javascript') || contentType.includes('text/js');
  }

  private isScriptLikeUrl(targetUrl: URL): boolean {
    const pathname = targetUrl.pathname.toLowerCase();

    return pathname.endsWith('.js') || pathname.endsWith('.mjs') || pathname.endsWith('.cjs') || pathname.endsWith('.jsx') || pathname.endsWith('.ts') || pathname.endsWith('.tsx');
  }
  ...
  private rewriteHtmlResourceUrls(html: string, targetUrl: URL): string {
    ...
    return html.replace(/\s(src|href|action)=("([^"]*)"|'([^']*)')/gi, (_match: string, attrName: string, wrappedValue: string, doubleValue: string, singleValue: string) => {
...
`;
  ...
  private patchWebpackPublicPath(scriptText: string, targetUrl: URL): string {
    ...
    nextText = nextText.replace(/(\b[a-zA-Z_$][\w$]*\.p\s*=\s*)(["'])(\/[^"']*)\2/g, (_match, prefix: string, quote: string, publicPath: string) => {
      const proxyPublicPath = this.createProxyPublicPath(publicPath, targetUrl);

      return `${prefix}${quote}${proxyPublicPath}${quote}`;
    });

    nextText = nextText.replace(/(\b__webpack_require__\.p\s*=\s*)(["'])(\/[^"']*)\2/g, (_match, prefix: string, quote: string, publicPath: string) => {
      const proxyPublicPath = this.createProxyPublicPath(publicPath, targetUrl);
  ...
  private collectResponseBody(proxyRes: http.IncomingMessage, onDone: (body: Buffer) => void, onError: (error: Error) => void): void {
    const chunks: Buffer[] = [];
  ...
  private sendBufferedResponse(
    ...
    const isScript = this.isScriptLikeUrl(targetUrl) || this.isJavascriptResponse(proxyRes.headers);
      ...
      this.collectResponseBody(
        ...
        (body) => {
          ...
          if (isScript) {
            ...
            patchedHeaders['content-type'] = proxyRes.headers['content-type'] || 'application/javascript; charset=utf-8';
  ...
  private safeSend(res: http.ServerResponse, statusCode: number, headers: http.OutgoingHttpHeaders, body?: string | Buffer): void {
